...
// 시험 둘(TestToolTableIsNine · TestInitializeAndToolsListRoundTrip)이 그것을 강제한다.
// 그리고 같은 절이 이 부류의 처방을 이미 정해 뒀다:
// **"규율은 응답에 싣는다 — 필요할 때만, 그 자리에서."**
//
// ★ 위 두 이름을 정확히 적는 이유: 앞선 판의 그 줄은 `TestToolTableIsSix` 를 인용했는데
// 그 이름은 레포에 없다(land 를 더하며 TestToolTableIsSeven 으로 개명됐고, 그 뒤
// label·amend 를 차례로 더하며 두 번 더 개명돼 지금은 TestToolTableIsNine 이다) —
// grep 하는 사람이 빈손이 됐다. 판정 자체는 안 뒤집혔다:
// 도구가 여섯에서 일곱이 된 것은 예산을 안 건드렸기 때문이고(늘어난 고정비는
// 이름 하나, 설명 90자 상한 그대로 — DESIGN §6), "예산을 쓰는 도구는 안 더한다"는
// 이 항목의 판정은 그래서 지금도 산다.
//
...
// 여기서 못박는 전부다. 도구 **개수**의 정본은 protocol_test.go 의 TestToolTableIsNine
// 하나여야 한다(정본이 둘이면 하나만 고쳐진 날 조용히 갈린다) — 그래서 개수는 여기서 안 잰다.
// ★ 다만 그 "하나"는 아직 의도지 사실이 아니다 — 지금 개수를 실제로 세는 시험은 셋이다:
// TestToolTableIsNine · TestInitializeAndToolsListRoundTrip · TestPickGainsItemIDsWithoutGrowingToolCount.
