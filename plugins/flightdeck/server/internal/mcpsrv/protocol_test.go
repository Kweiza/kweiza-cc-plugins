...
//
// 이 예산이 도구를 여덟으로 눌러 잡은 이유이고, 문구가 자라면 세션 시작 컨텍스트가 자란다.
// 그 여덟을 잠그는 것은 바로 아래 TestToolTableIsEight 이다(이름·순서까지 못박는다).
func TestInstructionsBudget(t *testing.T) {
	...
	// ★ 둘째 줄 추가(2026-08-21): 후속 트리아지. **같은 자리에서 같은 수술이 이미 성공한
	// 것이 위 줄이고**, 후속 축이 정확히 그 상태였다 — 상시 컨텍스트에 후속이라는 낱말이
	// 0바이트라, 거르는 기준이 세션에 닿는 길이 거절 응답 하나뿐이었다. 원장 실측(창
	// 2026-08-11~08-21)에서 그 도달률이 1/338 이다. 규율 산문이 아니라 **한 구**다 —
	// 금칙어 셋을 안 쓰고, 기준 하나와 그 자리에서 할 일만 담는다.
