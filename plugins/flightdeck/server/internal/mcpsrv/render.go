...
func RenderBoard(v service.BoardView, opt BoardRenderOptions) string {
	...
	if v.OutOfWindow > 0 {
		...
		// 도구 수를 컨텍스트 예산 안에 눌러 잡는다, §6) — 없는 손잡이를 가리키는 문구는
		// 그 자체가 결함이다.
		//
		// ★ **수를 안 적는다.** 여기 "7개"가 2026-08-05 이래 낡은 채로 있었고, 그 거울인
		// render_test.go:1004 는 도구가 늘 때마다 고쳐져 **시험과 코드가 서로 다른 수를
		// 말하고 있었다.** 낡은 수를 찾는 grep 이 "아홉"·"9개" 를 패턴으로 잡으면 이 자리는
		// 원리적으로 안 걸린다 — 관문의 전수는 정규식의 전수일 뿐이다.
...
func RenderAdd(it model.Item) string {
	...
	//
	// 되돌리는 길도 같은 줄에 적는다. MCP 표면에는 move 가 없고(설계 §6 이 도구 수를
	// 컨텍스트 예산 안에 눌러 잡는다 — 여기에도 그 수를 적지 않는다: 적는 자리마다
	// 도구가 늘 때 따로 밀린다), 대신 §6 이 정한 방식이 이것이다:
	...
	fmt.Fprintf(&b, "프로젝트가 %s 가 아니어야 한다면 지금 되돌려라: `fd move %s --project <맞는 것>`\n",
		it.Project, it.ID)
	// ★ **본문 축의 두 수단이 이제 갈려야 한다.** 바로 위 줄이 프로젝트 축에 대해
	//   되돌리는 길을 적는 것과 같은 자리·같은 이유로 여기 싣는다 — 고칠 수 있는
	//   축이 한 화면에 있어야 "무엇을 고칠 수 있나"가 갈리지 않는다(DESIGN §11).
	//
	//   ★★ 실측(2026-09-16) — 이 자리가 "본문은 나중에 못 고친다"라고 말한 채로
	//   amend 표면이 열려, 항목을 만들 때마다 사람에게 **거짓**이 나갔다(코디네이터가
	//   `fd add` 를 치고 그 줄을 실제로 받아 잡았다). §11 이 경고한 "부재를 주장하는
	//   문장은 코드가 바뀌면 조용히 거짓이 된다"가 이 자리에서 그대로 실현된 것이다 —
	//   `item_body_immutable_test.go` 를 지우지 않고 `item_body_single_writer_test.go` 로
	//   전환한 것과 같은 이유로, 여기서도 문장을 지우지 않고 **전환**한다.
	//
	//   그래도 note 는 지우지 않는다 — 둘은 다른 일이다. `amend` 는 본문 자체를 고치고
	//   (옛 값은 `item_revision` 에 추가 전용으로 남는다), `note` 는 그 위에 판단을
	//   얹는다(원문은 그대로, 집을 때 전문으로 온다). 오타·리네임 추종처럼 본문 자체가
	//   틀린 경우는 amend, "왜 이렇게 했는지"를 남기고 싶은 경우는 note 다. 하나만
	//   말하면 반대쪽이 사라진다 — 이 두 줄이 없으면 세션은 amend 의 존재를 몰라 note 로
	//   정정을 쌓아 원장을 잡음으로 불리거나, note 만 알아 본문 자체는 안 고치고 넘어간다.
	//
	//   ★★ 표기(2026-09-16 재리뷰) — 두 줄 다 **`동사(축: 값)` 의사-호출 표기**로 통일한다
	//   (`note(item_id: …)` ·`RenderFinish` 의 `pick(item_ids=[…])` 와 같은 이 저장소의
	//   기존 관용구). 바로 위 `fd move` 줄이 리터럴 셸 명령인 것은 **move 에 MCP 도구가
	//   없어 그것이 유일한 표현**이기 때문이다(그래서 예외로 남는다) — amend 는 이제
	//   아홉째 MCP 도구라 그 예외가 안 선다. 리터럴 `fd amend …`로 두면 MCP 세션은
	//   자기가 못 치는 셸 명령을 안내받고, note 만 의사-호출로 두면 CLI 사용자는 셸에
	//   없는 문법을 안내받는다 — 표면마다 같은 답이어야 한다는 이 작업의 주제와 어긋난다.
	//
	//   ★★ 긴급성(2026-09-16 재리뷰) — "지금 고쳐라"를 안 쓴다. 그 급함은 `fd move` 줄의
	//   것을 그대로 복사한 것이었는데, move 는 id 가 그 자리에서 브랜치 이름이 되어 창이
	//   좁지만 amend 는 기한이 없다(`store/amend.go` 가 닫힌 항목에도 연다 — "닫힌
	//   항목의 틀린 본문은 note 로도 못 닿는다"가 이 표면을 연 근거 중 하나였다). 급함을
	//   옮겨 붙이면 "지금 안 고치면 못 고친다"는 옛 전제가 낱말만 바뀐 채 남는다.
	fmt.Fprintf(&b, "본문이 틀렸으면 `amend(item_id: \"%s\", reason: <사유>, …)` 로 고쳐라"+
		"(옛 값은 개정 이력에 남는다 — 항목이 닫힌 뒤에도 된다, 급하지 않다).\n", it.ID)
	fmt.Fprintf(&b, "판단만 남기려면 `note(item_id: \"%s\")` 를 써라(집을 때 전문으로 온다).\n", it.ID)
	return b.String()
}
