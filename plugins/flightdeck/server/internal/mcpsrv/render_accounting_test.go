...
func TestToolPickStaysCleanWhenEveryIDIsAccountedFor(t *testing.T) {
	...
	// ★ **양성 대조를 함께 둔다 — 실측으로 필요해졌다.** 이 시험은 부정 단정 하나뿐이라
	// pick 본문을 통째로 비우는 변이에도 **초록이었다**: 빈 본문에는 경고도 없기 때문이다.
	// 본문이 실제로 나왔다는 것을 함께 못박아야 "경고가 안 붙었다"가 뜻을 가진다.
	if !strings.Contains(bodyOf(t, text), "브랜치: ok-a") {
		t.Fatalf("선점 본문이 응답에 없다 — 경고 부재만으로는 빈 응답도 이 시험을 통과한다:\n%s", text)
	}
	// ★ 부정 단정은 **응답 전체로 둔다 — 여기서 좁히면 시험이 약해진다.** 경고가 꼬리로
	// 새어 나가는 날 본문만 보는 단정은 그것을 못 잡는다. 좁히기가 항상 옳은 것은 아니다:
	// "여기 있어야 한다"는 좁히고, "어디에도 없어야 한다"는 넓게 둔다.
