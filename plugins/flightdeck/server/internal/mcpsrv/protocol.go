...
// ★ 첫 줄 갱신(2026-08-12, Task 13): "랜딩 전에 `land` 로 줄을 선다"를 더했다 —
// lane-turn 처방 큐가 전 기간 0건이었던 원인이 이 자리였다. pick·note·finish 셋만
// 이름을 대던 시절에는 land 라는 낱말 자체가 세션 시작 컨텍스트 어디에도 없었고,
// 세션은 finish 뒤에 land 로 줄을 선다는 규약을 스스로 찾아낼 길이 없었다 — 문이 없었다.
// ★ 둘째 줄 추가(2026-08-21): 후속 트리아지. **위 문단과 같은 진단이고 같은 수술이다** —
// 후속에도 문이 없었다. 거르는 기준(service.TriageGuidance)이 세션에 닿는 길은 거절 응답
// 하나뿐이었고, 원장 창 2026-08-11~08-21 에서 그 도달률이 1/338 이다(관문 1회 발화 :
// 창작 후속 338건). 여기 실은 것은 규율 산문이 아니라 **한 구**다 — 기준 하나와 그 자리에서
// 할 일. 예산도 확인했다: 이 상수 137→182 runes / 상한 300.
