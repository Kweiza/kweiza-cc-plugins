...
// 발화 대비 도달 가능 배수는 대화로 접어도 여러 배로 남는다(같은 날 롤링 24시간 창에서
// 2.0~5.5배를 봤다). 그 남은 몫은 갈림이 아니라 **처방을 받고 판단을 하나도 안 남긴 대화**
// 이고, 그것이 이 지표가 원래 재려던 것이다. 이 배수는 표본이 얇을수록 크게 흔들리므로
// 한 시점 값으로 판정하지 마라 — 며칠 치를 같은 방식으로 재야 방향이 보인다.
//
