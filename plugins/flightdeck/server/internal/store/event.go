...
//
// ★ 같은 부류의 축이 둘 더 있다. 이름만 붙여 둔다 — **최신 창 하나를 재는 실호출**에는 오차를
// 안 만들지만(둘 중 하나는 그 조건 밖에서 실제로 만든다 — 아래 ★), 이름이 없으면 다음 사람이
// 본문을 읽다 세 번째로 같은 자리를 다시 발견하게 된다.
//
...
// 오차 분석에는 중립이다 — **다만 그 중립은 두 값이 같은 시점일 때뿐이고, 롤링에서는 깨진다
// (아래 ★).** 유일 호출자가 finish 직후에 부르는 finish_balance.go 라 마무리와 조회 사이에
// 낄 수 있는 add 도 작다. 상한을 걸고 싶어지면 그 순간 분모의 상한과 **같은 id**로
...
//
// ★ **그 둘 중 add 상한은 중립이 아니었다 — 롤링/시계열로 과거 창을 재구성하면 거짓이고,
// 실측으로 그렇다.** 과거의 한 창을 다시 계산하는 순간 그 창보다 **뒤에 들어온 add** 가
// [sinceID, ∞)를 타고 그 창의 분자로 들어온다. 실호출(finish_balance.go)이 안 겪는 것은 이
// 축이 구조적으로 안전해서가 아니라 **그 호출이 언제나 최신 창이라 "미래"가 없기 때문**이다.
// 5차 실측(2026-08-12 · DESIGN §10)이 같은 원장·같은 창 20 으로 둘 다 냈다: 위 끝을 **창의
// 최신 센 마무리 id** 로 건 쪽은 1 미만 창이 95개(롤링 최소 0.350)이고, 함수 그대로(상한 없음)
// 는 28개(0.450)다 — 그 판정은 어느 쪽으로 재도 같았지만 수는 이만큼 갈린다. 그러니 롤링을
// 재는 사람은 위 끝을 **창의 최신 센 마무리 id** 로 걸고(= 분모의 상한과 같은 id · 위 문단이
// 시키는 그것이다), **어느 쪽으로 잰 수인지 함께 적어라**(적지 않아 못 되찾은 수가 이미 있다
// — DESIGN §10 5차 ③ 의 "롤링 최소 1.05").
//
// 그래서 이 축은 위 **LIMIT 축과 다른 부류**다. LIMIT 은 관측과 참값이 **같은 n행 집합**을
// 보므로 언제나 중립이고(구조적), add 상한은 **비교하는 두 값이 같은 시점일 때만** 중립이다
// (조건부). 두 이름이 같은 목록에 나란히 있다고 같은 보장을 주는 것이 아니다.
// **함수는 안 고친다** — 유일 호출자가 최신 창만 재므로 지금 그대로가 옳다. 이 문단은 그
// 코드가 아니라 **원장을 손으로 재는 사람**에게 거는 조건이다.
//
