...
// 1.30 이 finish_balance.go · mcpsrv/render.go · event_reproduction_test.go 에도 있다.
//
...
// **관측 < 참값 ⟺ c+k+a < 참값** 이다.
//
// ★ **이 열거가 전부라는 보장은 없다 — 이 문단은 세 번 틀렸다.** 처음엔 분모(2)를 잊고
// "과대로만 틀린다"고 적었고, 다음엔 하한이 들이는 add(3)를 잊고 "count=0 이면 항상 과소"라고
// 적었고, 그다음엔 롤백된 add(4)를 잊고 "항은 셋"이라고 적었다. 매번 잊은 항을 설명하는 문장이
// 같은 파일 안에 이미 있었다. 다음 사람은 **다섯째 항이 있다고 가정하고** 읽어라 — 세는 규칙을
// 고칠 때 분자·분모·하한·add 넷을 전부 짚었는지 확인하는 것이 이 문단의 쓸모다.
//
...
// 있어도 관측은 17/20=0.85 다. (셋이 다 0이고 그 창의 F+A 도 0이면 관측=참값=0 이라 어느
// 쪽으로도 안 틀린다.)
//
// ★ **그 바닥 갈래는 가설이 아니다 — 다만 근거는 tx 키가 0건인 것이 아니다.** tx 가 0건이라는
// 사실이 말하는 것은 "지금 창의 바닥이 롤백이었는지 **배제할 수 없다**"이지 실재한다가 아니다.
// 실재의 근거는 따로 있다: 알려진 롤백 4건은 각각 뒤에 같은 프로젝트 마무리가 75~153건 쌓였고
// (2026-08-10 01:58 KST · mode=ro 로 연 원장), 창이 20이므로 롤링 창이 굴러가는 동안 **넷 다
// 한 번은 창의 바닥에 앉았다.** 그때의 R 이 이 갈래를 실제로 겪었다. 오늘의 두 창에서는
// 안 보인다 — 그 4건은 지금 하한 아래다(DESIGN §10 한계 ②).
//
