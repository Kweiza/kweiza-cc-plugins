...
// 다만 event_reproduction_test.go 의 다섯 시험은 롤백을 전부 창 **끝**에 둔다 — 한가운데
// 갈래를 무는 시험은 없고, 이 문장이 그 자리를 대신한다.
//
...
//
// ★ **그 접기가 R 을 틀리게 하는 방향은 하나가 아니다 — 그 행의 count 와 지금 R 의 대소로
// 갈린다.** 표시 없는 롤백은 예전처럼 세어지는데, 그 행은 count 를 분자에 더하는 **동시에
// 분모에도 1을 더한다**(아래 out.Finishes++ 를 그대로 지난다). R=(후속+add)/마무리 에
// count=c 인 행 하나가 들면 R'=(F+c+A)/(D+1) 이므로 **R' < R ⟺ c < R** 이다 — count 가
// 지금 R 보다 작은 롤백은 R 을 **내린다**(count=0 이면 항상 내린다). 실측 롤백 4건 중
// 2건이 count=0 이라 그 과소 갈래는 실재한다: 창 20 · R=0.80 에서 count=0 행 하나가
// 0.80 을 0.76 으로 민다.
//
