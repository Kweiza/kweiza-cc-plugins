...
// 은 CASCADE **없이** `(project, item_id) → item(project, id)` 를 잡으므로 FK 위반이 난다.
// 이것은 `judgment_no_delete` + `judgment.project` FK 가 판단 있는 프로젝트의 삭제를 막는
// 것과 **같은 규율**이고, DESIGN §6 이 그 선례를 「② 판단이 있으면 원장이 거절한다」로
// 명문화했다. 여기 안 적으면 다음 사람이 FK 위반 원문을 보고 원인을 처음부터 찾는다.
