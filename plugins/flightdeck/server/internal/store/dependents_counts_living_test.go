...
//
// 기전: 옛 item_dependents 표는 addAfter(+1) · RemoveAfter(-n) · DeleteItem(-1) 에서만
// 움직였고 **기대던 항목이 done·dropped 로 닫혀도 안 줬다.** 그래서 아무도 안 기다리는
// 항목이 영영 "남이 기다린다"로 pick 의 1축을 이겼다. 그리고 그 틀림은 오류를 안 낸다 — 조용하다.
//
// ★ 그 표는 이제 죽었다(2026-08-22): 쓰기 셋을 걷고 증분 010 이 값을 비웠다.
// 전수 실측(143행, 예외 0)으로 그 n 이 실제로 센 것은 살아 있는 종속이 아니라
// **item_after 의 간선 수**였음이 확인됐다 — 위 세 건의 어긋남은 그 정의 차이의 그림자다.
// 이 시험이 재는 것은 그 표가 아니라 아래 파생 질의이므로, 표가 죽어도 축은 그대로 산다.
//
