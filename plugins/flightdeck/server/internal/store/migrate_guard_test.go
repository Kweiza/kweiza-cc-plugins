...
var destructiveExempt = map[int]exemption{
	...
	// 012 · 삼킴으로 죽은 판단↔항목 링크 21행 복구(INSERT … SELECT).
	//
	// ★ 008 의 사유("새로 만든 표를 채운다")를 복사하면 안 된다. 저쪽은 이 증분이 방금 만든
	//   빈 표가 대상이라 채우는 것 말고 할 수 있는 일이 없었지만, 여기는 **이미 8009행이
	//   살고 있는 표**에 행을 더한다. 그래서 축이 "빈 표를 채운다"가 아니라
	//   **"기존 행을 한 글자도 안 바꾼다"** 다.
	12: {
		ops: []op{opInsertSelect},
		why: "judgment_link 에 21행을 **더하기만** 한다 — 기존 8009행을 한 글자도 안 바꾸고, " +
			"judgment·item·item.close_reason 은 읽기만 한다(운영 사본 예행 2026-08-23: " +
			"judgment 4534 불변 · 오염 close_reason 20건 불변 · 링크 8009→8030). " +
			"되돌리기는 그 21행의 DELETE 다 — PK 가 (judgment_id, target_kind, target_id) 라 " +
			"정확히 지목되고, 지우면 이 증분 이전 상태와 동일해진다. **넣는 값이 결정적이다**: " +
			"각 항목 id 는 삼켜진 판단 본문의 꼬리에서 그대로 읽은 것이고 judgment 는 추가 전용이라 " +
			"(judgment_no_update·judgment_no_delete) 그 본문이 변하지 않는다 — 언제 다시 돌려도 " +
			"같은 21행이다. **멱등이다**: NOT EXISTS 가 이미 있는 링크를 거른다(사본에서 두 번 " +
			"돌려 8030 고정 확인). 신규 설치 DB 에서는 judgment 조인이 0행이라 아무것도 안 " +
			"들어간다 — 조건 없이 VALUES 로 밀면 judgment_id 의 REFERENCES judgment(id) 가 " +
			"FK 위반을 내 적용 자체가 실패하므로, 그 조건부가 opInsertSelect 를 부른 이유다. " +
			"판올림 전 VACUUM INTO 백업도 자동으로 뜬다. 근거 전문과 21쌍은 012 머리말에 있다.",
	},
}
