...
func TestJudgePathCoordinate(t *testing.T) {
	cases := []struct {
		...
		// ★ 드라이브 문자는 **알파벳일 때만**이다. 위 "a:b" 는 구분자 축(콜론 뒤에
		// 슬래시가 없다)이 막으므로 알파벳 축을 하나도 안 잰다 — 실제로 그 검사를
		// 통째로 지워도 전 스위트가 초록이었다(2026-08-10 돌연변이 실측).
		// 그 상태에서 "1:/foo" 같은 정상 POSIX 경로가 드라이브 절대경로로 거절된다.
		{"숫자 뒤 콜론·슬래시는 드라이브가 아니다", "1:/foo", true, "슬래시 좌표계"},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
