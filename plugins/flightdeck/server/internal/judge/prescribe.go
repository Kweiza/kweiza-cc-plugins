...
// 대기율이 1.06% → 8.3%(전 기간 434행) · 9.1%(최근 9일 187행)로, "10배면 대기 11%"라는
// 그 문단의 수와 자릿수가 같다. ρ 도 1.14% → **3.09%** 로 2.7배다.
// **틀린 것은 거기서 이어 붙인 추론이다**: ρ 가 오르면 발화가 따라 는다고 읽었는데,
// ρ 가 2.7배·대기율이 8배 오르는 동안 발화는 **1건에 그대로 멈춰 있었다.**
//
// **③ 진짜 기구 — 교집합이 좁은 것이 아니라 그 안에서 턴이 안 끝난다.** 이 축은
// 「맨 앞이다」 ∧ 「아무도 안 쥐었다」에서만 뜨고, 그 상태 **안에서 턴이 끝나야** 나간다.
// 실측 분모를 셋으로 갈라 재면:
//
//	맨 앞이 된 뒤에도 줄에 남아 있던 행        31건
//	  그중 아무도 안 쥔 시간이 실제로 있던 것   28건   ← 이 축이 뜰 수 있는 상태
