...
// 0건**이다(재측 2026-08-12 · `kind='prescribe'` **545건** 시점의 축 분포: overlap 310 ·
// unclaimed 120 · silent 74 · outside 41. 257건 시점 값이던 138·67·37·15 를 갱신한다).
// 코드도 상시가 아니라 줄 행마다 1회다(laneTurnPrescription 의 `LaneTurnRow <= 0`
// 조기 반환 + suppressed). **그래서 이 축의 접힘 위험은 아직 한 번도 실측된 적이 없다** —
// 아래 소실 기구는 다른 네 축에서 관측된 것이고, lane-turn 에 대해서는 코드를 읽어 세운 추론이다.
// 0 이 이어지는 **원인**은 laneTurnPrescription 독스트링에 적었다(기구가 아니라 이용률이다).
//
