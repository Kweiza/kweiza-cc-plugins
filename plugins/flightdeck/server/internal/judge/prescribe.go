...
//
// ★ **재측(2026-08-06) — 위 "35턴 중 2개"는 이제 낡았다.** `lane-turn` 이 축에 들어와
// 한 자리를 상시 먹은 뒤로 다시 쟀다(턴 = 같은 세션·같은 초의 prescribe 이벤트 묶음):
//
//	전 기간         턴 129개 중 접힌 턴 **15개**(11.6%) · 한 턴 최대 **7건**
//	lane-turn 이후  턴 9개 중 접힌 턴 3개(표본이 작아 비율은 안 쓴다)
//	접혀서 사라지던 축  overlap 11 · unclaimed 11 · silent 4 · outside 2
//
// 접힘은 드문 사건이 아니었고, 사라지던 것의 절반이 `unclaimed` 다 — silent 와 달리
// 해제 규칙이 없어 한 번 접히면 그 세션에서 영영 안 뜨는 축이다. 이 수치가 위 개정의 근거다.
// 상한 3 자체는 그대로 둔다: 소실이 막힌 지금 상한이 하는 일은 **한 턴에 읽을 양의 제한**뿐이다.
//
