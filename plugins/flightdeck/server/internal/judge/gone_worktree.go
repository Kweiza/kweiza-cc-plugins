...
//
// ★ 이것은 **관측에 근거한 판정**이다 — 관측이라고만 부르면 거짓이다. 좌표의 부재는 관측이지만
// 거기서 "이 카드를 닫는다"로 넘어가는 한 걸음은 판정이고, 이 파일이 그 걸음의 조건을 적는다.
// 다만 **생존 판정은 아니다.** 신호 나이·pid·무응답을 한 글자도 안 본다. 앞선 두 오판(죽었다고 본
// 세션이 6커밋 랜딩 · 419분 무갱신이 17초 전 살아 있음)은 둘 다 **세션의 행동**에서 죽음을
// 추론했다. 여기서 근거로 삼는 것은 **서버가 git 에서 직접 관측하는 좌표의 부재**다 — §4 신호
// 표의 `commit·push` 가 "클라이언트를 안 믿는 유일한 신호"인 것과 같은 부류다. 카드의 워크트리가
// git 목록에서 빠지고 디렉토리도 없으면, 그 카드는 **그 좌표에서는** 일할 수 없다.
//
// ★ 판정은 둘로 갈린다. 섞으면 "좌표가 없다"와 "없어도 닫으면 안 된다"가 한 사유 문자열로
// 뭉개져, 안 닫힌 카드를 보고 어느 쪽 때문인지 못 가린다.
//
//	WorktreeGoneCandidate · WorktreeGone — 좌표의 부재(git 목록 · 관례 자리 → 서버 파일시스템)
//	MayCloseGoneCard                     — 그 카드를 닫아도 되나(state · 선점 · mcp 카드 · 요청자)
//	PriorAutoCloseAllows                 — 카드당 한 번(원장)
//
...
//
// WorktreeGoneCandidate 가 이 함수로 두 자리를 구해 내고, 호출부(service)는 **그 값을** stat 해서
// GoneWorktreeFacts 에 싣는다. WorktreeGone 은 같은 함수로 다시 구한다 — 자리를 두 곳에서
// 따로 구하면 잰 경로와 판정한 경로가 조용히 어긋난다.
...
// GoneWorktreeCandidate 는 **문자열만으로** 가를 수 있는 앞 다섯 조건의 결과다.
type GoneWorktreeCandidate struct {
	OK     bool   // 다섯 조건이 다 섰다 — 이제 서버에서 잴 차례다
	Tree   string // 잴 관례 워크트리 루트
	Home   string // 그 루트를 담는 목록 워크트리
	Reason string // OK 가 아니면 왜 아닌지. OK 면 빈 문자열
}

// WorktreeGoneCandidate 는 좌표 판정의 **앞 다섯 조건**이다. 순수 함수다.
//
// ★ 왜 따로 떼나 — 평시 비용이다. 파생마다 모든 카드가 이 판정을 지나는데, 살아 있는 카드는
// 거의 전부 조건 2(목록에 있다)에서 걸린다. 서버 파일시스템을 재는 것(lstat·stat)은 이
// 다섯이 다 선 카드에만 하면 되고, 그러면 평시 추가 비용이 문자열 비교뿐이다. WorktreeGone 이
// **이 함수를 그대로 부르므로** 조건의 순서와 문구는 한 자리에만 산다.
//
...
//     저장소·다른 머신의 경로다)
func WorktreeGoneCandidate(worktree string, listOK bool, listed []string) GoneWorktreeCandidate {
	if !listOK {
		return GoneWorktreeCandidate{Reason: "git worktree list 를 못 읽었다 — 목록이 없으면 부재를 말할 근거가 없다(마운트 밖·권한·일시 오류일 수 있다)"}
	}
	wt := strings.TrimSpace(worktree)
	if wt == "" {
		return GoneWorktreeCandidate{Reason: "카드의 워크트리 값이 비었다"}
	}
	if listedWorktree(wt, listed) {
		return GoneWorktreeCandidate{Reason: "git worktree list 에 있다"}
	}
	tree, home := GoneWorktreeCoords(wt, listed)
	c := GoneWorktreeCandidate{Tree: tree, Home: home}
	switch {
	case tree == "":
		c.Reason = "관례 워크트리 자리(.flightdeck/worktrees/<이름> · .claude/worktrees/<이름>) 안이 아니다 — 주 체크아웃이거나 이 저장소의 워크트리였다는 근거가 없다"
	case listedWorktree(tree, listed):
		c.Reason = fmt.Sprintf("담는 워크트리 %s 가 git worktree list 에 있다 — 카드는 그 하위 디렉토리일 뿐이다", tree)
	case home == "":
		c.Reason = fmt.Sprintf("%s 를 담는 워크트리가 git worktree list 에 없다 — 이 저장소의 좌표라는 근거가 없다", tree)
	default:
		c.OK = true
	}
	return c
...
//
// 조건 일곱이 **전부** 서야 Gone 이다. 앞 다섯은 WorktreeGoneCandidate 이고, 뒤 둘은 서버가
// 잰 값이다.
//
//  6. 담는 워크트리가 서버에서 보인다 — 안 보이면 마운트 밖이라 부재를 못 잰다
//  7. 관례 루트가 서버에서 없다(ErrNotExist) — 있거나 못 쟀으면 안 닫는다.
//     ★ 이것이 **"목록에는 없는데 살아 있는" 카드를 막는 유일한 층**이다. git 은 gitdir 를
//     못 읽는 워크트리(관리 디렉토리가 치워졌거나 repair·move 로 다시 쓰이는 순간)를 목록에서
//     조용히 빼고 0으로 끝난다 — 그때 디렉토리는 살아 있다.
func WorktreeGone(f GoneWorktreeFacts) GoneWorktreeVerdict {
	c := WorktreeGoneCandidate(f.Worktree, f.ListOK, f.Listed)
	v := GoneWorktreeVerdict{Tree: c.Tree, Home: c.Home, Reason: c.Reason}
	switch {
	case !c.OK:
	case f.HomePresence != PathPresent:
		v.Reason = fmt.Sprintf("담는 워크트리 %s 가 서버에서 안 보인다 — 마운트 밖이면 부재를 못 잰다", c.Home)
	case f.TreePresence == PathPresent:
		v.Reason = fmt.Sprintf("%s 가 서버에 아직 있다 — git 목록에서만 빠졌다", c.Tree)
	case f.TreePresence != PathAbsent:
		v.Reason = fmt.Sprintf("%s 를 서버에서 못 쟀다 — 없다고 볼 근거가 아니다", c.Tree)
	default:
		v.Gone = true
		v.Reason = fmt.Sprintf("git worktree list 에 없고 서버에도 없다(%s · 담는 워크트리 %s)", c.Tree, c.Home)
	}
	return v
}

// GoneCardGuards 는 좌표가 사라진 카드를 **닫아도 되는지** 판정하는 데 필요한 사실이다.
// 전부 ListLive 가 이미 싣고 오는 값이라 조회가 안 는다.
type GoneCardGuards struct {
	State  model.SessionState
	Claims int  // 이 카드가 쥔 선점 수(프로젝트 무관)
	IsSelf bool // 이 판정을 부른 요청이 바로 이 카드에서 왔다
	// HasMCPSignal 은 이 카드에 mcp 신호가 한 번이라도 찍혔는지다.
	HasMCPSignal bool
}
...
//     안 보인다.
//   - **MCP 카드는 안 닫는다 — 되살릴 길이 없다.** `fd mcp` 는 띄운 자리의 cwd 로 카드를 열고
//     (그 자리가 관례 워크트리면 카드 워크트리도 거기다) 프로세스당 **한 번만** 연다
//     (mcpsrv 의 ensureSession). 그 뒤 도구마다 찍는 mcp 신호는 state 를 안 건드리고(Tx.Beat),
//     그 세션이 워크트리를 지운 뒤 main 에서 계속 일하면 훅은 **다른 3중키**라 이 카드를 안
//     연다. 그런데 그 세션의 pick 은 이 카드 id 로 선점을 쥔다 — Pick 은 세션 state 를 안 보므로
//     **done 카드가 선점을 쥐는** 상태가 된다(설계 §4 가 어느 경로로도 막겠다던 바로 그것).
//     요청자 보호는 그 카드가 **부르는 동안**만 막고 호출과 호출 사이는 못 막는다.
//     훅이 세운 카드에는 mcp 신호가 없으므로 이 판정이 막으려던 사례(훅 유령)는 그대로 닫힌다.
//   - **요청한 카드 자신은 안 닫는다.** 지금 부르고 있다는 것이 그 카드가 살아 있다는 관측이다.
//
// ★ 앞선 자동 닫기(카드당 한 번)는 여기서 안 본다 — 원장 조회라 비싸다. 이 판정이 선 카드에만
// PriorAutoCloseAllows 를 부른다. 선점·blocked 유령이 남아 있는 동안 파생마다 카드마다 원장을
// 치지 않게 하려는 순서다.
func MayCloseGoneCard(g GoneCardGuards) (bool, string) {
	...
	if g.HasMCPSignal {
		return false, "mcp 신호가 있는 카드다 — MCP 는 프로세스당 한 번만 카드를 열어 닫히면 되살릴 길이 없다"
	}
	if g.IsSelf {
		return false, "이 요청을 보낸 카드다 — 지금 부르고 있다는 것이 그 카드가 살아 있다는 관측이다"
	}
	return true, "active · 선점 0건 · mcp 신호 없음 · 요청자 아님"
}
...
const (
	...
	PriorAutoCloseSeen                          // 읽었고, 자동 닫기 행이 있었다
)

// PriorAutoCloseAllows 는 **카드당 한 번** 한도를 판정한다. 순수 함수다.
//
// 자동으로 닫혔는데 다시 active 라면 그 3중키로 열기가 왔다는 뜻이고, 그 관측이 이 판정을
// 이긴다. 한도가 없으면 사라진 cwd 에서 신호를 보내는 세션이 신호와 파생마다 열림·닫힘을
// 오가고, 닫힐 때마다 지울 수 없는 event 행이 쌓인다.
//
// ★ 이것은 **미리 거르기**다. 한도의 정본은 쓰기 문장 안의 같은 조건이다
// (store.Tx.CloseSessionWhoseWorktreeIsGone) — 이 조회와 그 쓰기 사이에 다른 파생이 닫고 훅이
// 되살릴 수 있다. 여기서 거르는 이유는 되살아난 유령마다 파생 때마다 쓰기 잠금을 잡지 않게
// 하려는 것뿐이다.
func PriorAutoCloseAllows(p PriorAutoClose) (bool, string) {
	switch p {
	case PriorAutoCloseNone:
		return true, "앞선 자동 닫기 없음"
	case PriorAutoCloseSeen:
		return false, "이미 한 번 자동으로 닫혔다가 다시 열렸다 — 그 좌표로 열기가 왔다는 관측이 이긴다"
	default:
