...
// 열린 16건 중 10건을 한 묶음으로 끌어왔다.
// ★ 이 줄은 §0 의 **하위번호**를 근거로 달고 있었다. DESIGN 에 그런 번호가 없다 —
// 거기 번호가 붙은 것은 병목 일곱(1=대시보드 … 7=게시판 조회)이고 이 실측과 무관하다.
// 이 수치는 이 함수를 쓰면서 이 저장소에서 직접 잰 것이라 인용할 상위 문서가 없다.
//
...
//
// ★★ **이 축은 `finish` 가 만든 후속과 이은 항목을 구별하지 않는다 — 그것이 판정이다**
// (2026-08-11). 이 색인의 원천은 `judgment_link`(target_kind='item') 한 표이고, 후속을
// **새로 만들 때**와 이미 있는 항목을 **이을 때** 그 표에 같은 모양의 행이 들어간다.
// 그래서 잇기가 생긴 뒤 이 축의 입력이 넓어졌고, 그 사실이 어디에도 안 적혀 있었다.
//
// 원하는 동작인가 — **그렇다.** 근거 셋이고 앞의 둘이 결정적이다:
//
//	① **이을 수 있는 것의 자격이 이미 좁다.** service.sessionSpawnedOpen 은 "이 항목의
//	   선점 시각 이후 · 이 세션의 item.add 이벤트 · 아직 열린 항목"만 낸다. 즉 이어지는
//	   것은 **그 세션이 그 작업에서 스스로 만든 항목**뿐이다. 남의 항목도, 오래 묵은 큐
//	   항목도 자격 밖이다 — 이 축을 걱정한 항목의 전제("이미 큐에 있던 항목이 섞여
//	   들어온다")는 그 검사가 막는다.
//	② **그래서 이은 항목은 새로 만든 후속과 성질이 같다** — 같은 세션 · 같은 작업 ·
//	   같은 판단에서 나왔고, 다른 것은 **만들어진 시점**뿐이다(finish 호출 때 vs 그 전에
//	   add 로). 형제 축의 뜻이 "한 사람이 한 자리에서 둘을 함께 생각했다"이므로 둘 다
//	   부합한다. 시점으로 그것을 가르면 같은 사실이 도구 호출 순서에 따라 달라진다.
//	③ **묶음이 커진다는 우려는 실측이 반박한다**(2026-08-11, 이 저장소 원장): 열린 항목을
//	   가리키는 판단 12개 중 둘 이상을 가리키는 것은 **2개**이고 각각 정확히 2건이다.
//	   ①의 자격 때문에 구조적으로도 못 커진다 — 한 세션이 한 선점 동안 만든 것이 상한이다.
//
// **잇기를 이 축에서 빼지 않았다**(그 항목의 선택지 ⓓ). 뺄 수는 있다 —
// `item.followup_linked` 이벤트가 잇기를 원장에서 구별하므로 축이 존재한다. 안 뺀 이유는
// 그것이 **세션이 명시적으로 선언한 관련을 추천기만 모르게** 만들기 때문이다. 잇기는 우연이
// 아니라 세션이 자기 판단에 그 항목을 매단 행위다.
//
// 한계 — 정직하게: **이 판정은 표본 0에서 내렸다.** 실측 시점까지 `item.followup_linked` ·
// `item.followup_skipped` 가 **전 역사 0건**이다(기능은 랜딩돼 있다). 그래서 위 ③의 수치는
// 잇기가 **없는** 상태의 형제 분포다. 잇기가 실제로 쓰이기 시작하면 그 수를 다시 재라 —
// 묶음 크기 분포가 이 판정을 뒤집는 유일한 축이고, 뒤집히면 ⓓ 가 준비돼 있다.
...
// (실측: 그 파일 하나로 4건이 서로 묶였다 — 이 저장소에서 직접 잰 값이다).
...
//
// ★ 수는 **하한이다.** LogEvent 는 쓰기 실패를 WARN 으로만 삼키고, BeginTx 가 실패한
// 트랜잭션은 이벤트를 예약조차 안 하므로, 원장에 안 남은 마무리가 있을 수 있다.
// 문구가 "이상"이라고 말하는 이유가 그것이다 — 정확한 수로 읽히면 "0건이니 안전하다"가
// 관측이 아니라 추측이 된다.
// (2026-08-09 에 사유 하나가 빠졌다: "클라이언트가 끊긴 마무리는 안 남는다"였는데
// store.flushCtx 가 흘리기 ctx 에서 취소를 떼면서 그 갈래가 닫혔다.)
//
...
// ①이 상수이고, 그 상태에서 ③이 실질 1차 키가 되는데 최고령이 단독이었다.
// ★ 이 줄도 §0 의 하위번호를 달고 있었고 같은 이유로 뺐다 — 위 주석 참조.
...
//	선점을 쥔 시간만큼이다 — 며칠 묵은 큐 항목이 이 경로로 끌려 올라오는 일은 없다.
//	위 실측의 "7.5h 이상 묵은 17건"이 구제되는 축이 **아니다**. 그쪽은 여전히 기아 갈래가
//	담당한다.
//
// 그리고 그 완화 폭은 아직 관측되지 않았다 — 잇기 실물 0건이다. 쓰이기 시작하면 여기가
// 재측정 대상이다.
//
