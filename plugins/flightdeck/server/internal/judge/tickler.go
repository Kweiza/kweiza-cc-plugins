...
// 무엇보다 `label` 로 **나중에 고칠 수 있다** — 재측해서 기한이 밀리면 갱신해야 한다.
//
// ★★ 갱신(2026-09-16) — 본문·제목도 이제 `amend` 로 고칠 수 있게 됐다(§11). 그래도
// 기한은 여전히 꼬리표 축이 맞다: `fires:` 접두 + 고정 표기(`FiresDateLayout`)라
// `FiresOn` 이 **정확히 파싱**할 수 있는데, 자유 산문인 본문에 같은 값을 박으면 꺼낼 때
// 자연어를 다시 해석해야 하고 그 파싱이 실패하는 날 이 표시 축 전체가(경고 없이) 죽는다
// — 표시-전용 규약(설계 §8)이 구조화된 자리 하나(꼬리표)에만 기대는 것도 그래서다.
// 그리고 `label` 은 그 축 하나만 건드리는 좁은 동사라 재측 한 번이 본문의 다른 내용을
// 건드릴 위험이 없다 — `amend` 로 옮기면 title·paths 가 같은 호출에 함께 얹혀
// 재측하려던 사람이 실수로 다른 축까지 바꿀 문이 열린다.
//
