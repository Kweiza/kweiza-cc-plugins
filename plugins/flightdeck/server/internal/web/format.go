// Package web 는 읽기 전용 서버 렌더 대시보드다. HTML 한 장, 섹션 6개, 쓰기 버튼 4개.
//
// 이 패키지가 지키는 것 넷:
//
...
//     구조로 막는다(설계 §6).
//  4. **파생물에 손대는 쓰기 폼을 만들지 않는다.** 쓰기는 넷뿐이고(선점 회수·항목 폐기·
//     레인 정지/재개·잡 우회 기록) 전부 사유가 필수다. 뒤 둘은 Tier B 라 **비활성 버튼으로
