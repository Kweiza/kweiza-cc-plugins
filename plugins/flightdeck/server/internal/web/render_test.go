...
func TestWriteFormsAreAtMostFourAndAllRequireReason(t *testing.T) {
	...
	//
	// ★ 로그아웃과 표시 축 폼은 이 셈에서 뺀다. 이 관문이 세는 것은 **파생물에 쓰는 폼**이고
	// (actions.go 머리의 그 규율), 로그아웃은 쿠키 하나를 지울 뿐 원장에도 파생물에도
	// 아무것도 안 남기고, 표시 축 폼은 원장에 쓰지만 그 컬럼이 표시 계층에만 닿는다(위 ★).
	// 상한을 올려서 통과시키면 다음 사람이 여섯째 폼을 넣을 때 "저번에도 올렸잖나"가
	// 근거가 되고, 그러면 이 락이 막으려던 증식이 한 칸씩 열린다.
	...
	// 로그아웃과 표시 축 폼은 파생물 쓰기가 아니라 이 셈에서 뺐다.
