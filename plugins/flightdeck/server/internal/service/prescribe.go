...
func (s *Service) Prescriptions(ctx context.Context, sessionID string) (PrescribeResult, error) {
	...
	//
	// ★ **다턴 배수도 쟀다(2026-08-19) — 매 턴 상한만큼이 맞다.** 08-12 에는 상한 3을 넘겨
	// 접힌 턴이 1개(6개 접힘)뿐이고 그 세션에 뒤 턴이 없어 표본이 0이었다. 그 뒤 일주일에
	// prescribe_folded 가 16→37, 4키+ 가 1→7 로 늘어 발화 조건(4키+ 이면서 뒤 턴 있음)이
	// **3건** 찼다.
	//
	// 완전 배수 24건에서 **턴당 복귀가 상한 3을 한 번도 안 넘었다**(분포 {1:14, 2:9, 3:5}).
	// 5개 접힘 둘 다 **정확히 두 턴**에 3+2 로 빠졌고, 같은 Since 연쇄도 이번엔 **2가 4건**
	// 나왔다(08-12 은 전부 1) — 연쇄 1 이 "안 걸렸다"가 아니라 "한 턴에 말랐다"였음이 여기서
	// 확인된다. 복귀 지연 중앙 13.7분.
	//
	// ★ **다만 "3개 이하는 한 턴에"는 조건부다.** 3개 접힘 1건이 두 턴 걸렸다. 원인은 상한이
	// 아니라 그 턴에 새로 뜬 처방이 앞자리를 가져간 것이다 — 새것이 낀 배수 턴 8개 중 실제로
	// 밀림이 생긴 것은 2개다.
	//
	// **접힘은 순서에 아무 영향을 안 준다.** FoldPrescriptions 는 조립된 목록의 앞 3개를 자를
	// 뿐이고(ps[:PrescribeMax]), 순서는 그 앞의 축 순서와 축 안 정렬이 정한다(overlap 은 상대
	// 세션 id 오름차순 — judge/prescribe.go 의 sort.Slice). 밀린 것에 우선권이 없다는 뜻이라
	// 원리적으로는 id 가 큰 상대가 계속 밀릴 수 있다. 다만 id 가 ULID 라 **오래된 상대가 먼저
	// 나가는 방향**이고, 관측된 기아는 0건(최대 2턴)이다. 바꾸려면 여기가 아니라 그 정렬이다.
	//
	// PrescribeMax 를 다시 볼 근거는 **이번에도 안 생겼다.** 미루는 것이 한두 턴이고 그 안에
	// 전부 복귀하므로 상한이 손해를 쌓지 않는다. 상한을 올리면 한 턴에 나가는 양이 늘 뿐이고,
	// 접힘이 애초에 상한을 넘긴 턴은 37건 중 7건(19%)이다.
