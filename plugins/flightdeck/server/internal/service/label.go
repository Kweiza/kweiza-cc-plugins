...
//
// ★ 범위를 **꼬리표 한 축으로 못박는다.** title·body·paths 는 `amend`(2026-09-16 에
// 열렸다, DESIGN §11)가 전담하고, 선행(after)의 사후 수정은 여전히 §11 이 "안
// 만든다"로 판정했다 — 두 동사에 걸쳐 "무엇을 고칠 수 있나"가 갈리면 그 차이를
// 아무도 못 따라간다는 것이 move 가 프로젝트 한 축으로 못박은 이유·cutAfterRequest 가
// "전용 동사"라고 적은 이유와 같다. 이 표면은 amend 의 세 축도 선행도 안 건드린다.
