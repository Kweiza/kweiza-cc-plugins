...
//
// ★ **"물으면 정해지는 것" 부류를 더했다(2026-08-21).** "전제 미확정"의 상당 부분이 조사가
// 필요한 것이 아니라 **한 번 물으면 끝나는 것**이었다. 실측(창 2026-08-11~08-20): 결정 지시형
// 강신호 155건 중 손검증 14건(+경계 2)이 그 부류이고, 그 14건은 열림 64% · 나이 중앙 6.3일
// (전체 3.0일의 2.1배)로 굶는다. 반대로 **답이 온 5건은 전부 1시간 안에 코드 0줄로 닫혔다.**
//
// ★ **도구 이름을 안 박는다.** fd 에는 세션→사람 채널이 없다 — note(kind='ask') 는 FilterNotes 가
// self 를 빼서 정의상 다른 세션에게 가고(기존 1,439건의 수신자 표기가 전부 세션이다), 훅의
// additionalContext·decision:block 은 둘 다 모델이 읽는다(후자의 실측은 "사람 개입 없이 턴을
// 되살렸다"였다). 그리고 fd 는 세션의 대화형 여부를 모른다 — 근사인 signal 'prompt' 는 표시
// 전용이고 처방 수신 세션 252건 중 16건(6.3%)이 그 신호가 0이다. 그래서 **행위로만** 말하고
// 못 묻는 갈래의 행동도 함께 적는다 — 한쪽에서만 참인 문구는 나머지 세션에게 없는 통로를
// 가리키고, 그것은 이 레포가 결함으로 분류하는 부류다(mcpsrv/render.go 의 RenderLand 주석).
...
**물으면 정해지는 것도 후속이 아니다** — 지금 물어서 그 자리에서 정하고, 못 물으면 무엇을 못 물었는지 본문에 적어라.
