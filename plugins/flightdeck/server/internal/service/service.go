...
const (
	...
	//
	// 배포 시각을 기준으로 자르는 것이 뜻은 가장 맞지만 원장이 "배포가 언제였나"를 모른다.
	// ★ **그 축을 담은 항목은 지금 없다.** fd-main-never-reaches-running-clients 가 담당인
	// 줄 알았는데 그것은 2026-08-05 에 dropped 로 닫혔고(핵심 수치가 사망했다), 분리된
	// 조각은 아웃박스 축이라 배포 시각과 무관하다. 이 항목의 후속으로 낸다 — 없는 담당을
	// 가리키면 아무도 안 여는 채로 남는다.
