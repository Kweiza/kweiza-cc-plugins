...
const (
	...
	//
	// ★ **넓히기 전에 파생 실패를 함께 재라.** 이 값은 화면 크기만 늘리는 게 아니다. 창 안의
	// 카드마다 UncommittedPaths·UncommittedDelta 가 돌고, 워크트리가 지워진 카드는 그 둘이
	// **함께** 죽어 축을 둘 낸다(board.go 의 두 d.fail). 그런 카드가 쌓이는 이유는 워크트리를
	// 지우고 세션 카드를 안 닫는 것이고, state 가 done 이 아니면 영영 파생 대상에 남는다.
	// 실측(2026-08-12 12:32 UTC · kweiza-cc-plugins · ListLive 질의 재현 + os.path.isdir):
	//
	//	창             2h(지금)  12h   24h   72h   168h   무한
	//	파생 대상            7    20    22    71    130    143
	//	worktree 없음        1     6     7    31     66     69
	//	실패 줄              2    12    14    62    132    138
	//	접은 뒤              1     6     7    31     66     69
	//
	// 2시간이 이것을 견디는 이유는 **자기제한**이다 — 지워진 워크트리도 2시간이면 창 밖으로
	// 늙어 나간다. 24시간부터는 접은 뒤에도 mcpsrv 의 줄 상한(3·6)을 넘어 대부분이 `… N줄 더`
	// 뒤로 숨는다. **접기(foldTwinFailures)가 산 이유가 그 규모에서 무너진다.**
	//
	// 2026-08-12 에 갈래 둘을 검토하고 **둘 다 기각**했다 — 근거를 다시 만들 필요는 없되,
	// 창을 넓히면 그 계산이 바뀌므로 **넓히는 사람이 둘을 다시 재야 한다**:
	//   · 죽은 경로를 파생에서 걸러내기 — 지금 창에서 얻는 것이 1줄과 5ms 뿐이다. 실패하는
	//     git 호출은 2.28ms 로 성공하는 호출 2.65ms 보다 **싸다**(status 가 파일을 stat 하지
	//     않는다). 197c8c9 가 같은 갈래를 "'못 읽었다'가 뜨는 것 자체는 옳다"로 이미 기각했다.
	//   · 워크트리가 사라진 카드를 자동으로 닫기 — 되살리는 경로가 Tx.OpenSession 하나뿐이고
	//     Tx.Beat 는 state 를 안 건드린다. 오판당한 세션은 신호를 계속 보내면서도 다음
	//     SessionStart 까지 보드에서 사라진다. claim 자동만료를 실측 2회로 기각한 것과 같은
	...
	//
	// ★ DefaultLiveWindow 를 재사용하지 않는다. 배선은 이미 있어 그쪽이 싸 보이지만
	// 2시간은 처방 표본이 죽는다 — 실측(2026-08-07 UTC)에서 2시간 창의 kweiza-cc-plugins 는
	// 1 · 0 · 0 이었다(도달 가능이 0이라 확인율이 **아예 안 나온다**). 24시간으로 넓히면
	// kweiza-cc-plugins 가 7 · 2 · 2, context-platform 이 13 · 4 · 4 로 읽힌다.
	// (stt-meetings 는 어느 폭으로도 안 산다 — 전 역사 표본이 카드 1장뿐이라 2시간에서도
	// 24시간에서도 0 · 0 · 0 이다. 창 선택의 근거가 아니라 표본 없는 프로젝트의 모습이다.)
	//
	// ★ 두 창은 재는 것이 다르다. DefaultLiveWindow 는 **세션이 살아 있나**를 보는
	// 표시 구간이고, 이쪽은 **규율이 어느 구간에서 지켜졌나**를 보는 계측 구간이다.
	// 하나로 묶으면 화면 창을 줄이는 순간 지표가 조용히 노이즈가 된다.
	//
	// 배포 시각 기준이 뜻은 가장 맞고, **그 축은 이제 원장에 있다** —
	// `Store.LastDeployAt`(`server.deploy` 이벤트. `fd serve` 가 기동할 때 자기 실행 파일을
	// 관측해 남긴다). 그런데 아직 `since` 로 안 쓴다. 이유는 표본이다: 배포 직후에는 창이
	// 0에 가까워 세 수가 전부 0이 되고(2시간 창에서 이미 본 모양이다), 배포가 오래됐으면
	// 창이 전 역사에 수렴해 절단이 뜻을 잃는다. 24시간과 어떻게 합칠지(둘 중 짧은 쪽?
	// 배포 뒤 경과가 일정 시간을 넘을 때만?)는 그 값으로 며칠 재 본 뒤 정한다 — 지금
	// 정하면 또 근거 없는 상수가 하나 는다.
	AckWindow = 24 * time.Hour
)
