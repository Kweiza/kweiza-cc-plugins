...
func TestLaneTurnFiresOnceWhenTheLaneBecomesMine(t *testing.T) {
	...
	//
	// ★ releaseLaneOrFail(=s.LandReport)이 위 ②의 어긋남("점유는 있는데 행이 없다")도
	// 놓는다 — LandReport 가 행을 먼저 읽어 없으면 laneNotMine 으로 직행하던 한때가
	// 있었지만(그 구조는 계획 결함이었다, 2026-08-12 정정 커밋 8150e9d), 지금은 행이
	// 없으면 ListHeld 로 내가 쥔 자원을 전부 걷어 반납하는 자가치유 갈래로 되돌아간다
	// (landing.go 의 LandReport 독스트링 참고). 이 시험이 store 직접 호출로 어긋남을
	// 만든 수법과 짝이 맞는 반납 경로다.
