...
// **어긋난 상태에서도 보고는 반납한다.**
//
// ★ **왕복 이력(2026-08-12).** Task 4 구현 중 이 시험을 한 번 `TestReportOfAHoldWithoutRowLeavesItForHumanRecovery`로
// 바꿔 정반대(치유 안 함·"reclaimed")를 잠갔었다 — 당시 브리프의 LandReport 코드가 행을
// 먼저 읽고 없으면 곧장 laneNotMine 으로 빠지는 모양이라, 그 구조에서는 행이 없는 순간
// "무엇을 반납할지" 자체를 몰라 자가치유가 원리적으로 불가능해 보였다. 그런데 그 구조는
// **계획 결함**이었다 — 이 어긋남이 실제로 벌어지면 `ReleaseLaneRow`도 살아 있는 행을
// 전제해서 프로그램적 탈출구가 0 이 됐다(sqlite3 직접 UPDATE 만 남는다, 2026-08-05 스펙
// §3 이 명시로 금지한 모양). 계획을 정정해(커밋 8150e9d) 행이 없을 때 `ListHeld`로 내가
// 쥔 자원을 전부 걷어 반납하는 자원 집합판 자가치유로 복원했고, 이 시험도 옛 이름·옛
// 단정 그대로 부활시켰다 — 아래 본문은 Task 4 이전 원본과 동일하다(assert 값 하나도
// 안 바뀜: LandReport 의 이 갈래는 여전히 out.RowID 를 안 채운다).
//
