...
// 리눅스가 `/proc/<pid>/exe -> …(deleted)` 로 그 사실을 그대로 보여 주고, exe.go 가 정확히
// 그 갈래를 찍는다. 다음 호출은 파일이 없는 것을 보고 다시 짓는다(웜 0.5~1.3초).
...
func PruneBinCache(entries []BinEntry, keep int, self string) []string {
	if keep < 0 {
		keep = 0 // 음수를 그대로 쓰면 아래 인덱스 비교가 조용히 '전부 유지'로 뒤집힌다
	}

	// self 에 붙은 커널 표식을 뗀다. 이 GC 가 한 번 돌고 나면 **이 프로세스 자신이**
	// 지워진 자리를 도는 상태가 될 수 있고(위 안전성 논의), 그때 os.Executable 은
	// "…/fd-… (deleted)" 를 준다 — 그대로 비교하면 self 보호가 아무 신호 없이 꺼진다.
	// 표식 상수는 exe.go 가 갖는다. 여기 문자열을 다시 적지 않는다.
	selfPath := ""
