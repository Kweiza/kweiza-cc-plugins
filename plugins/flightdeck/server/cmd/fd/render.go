...
func RenderPrescriptions(shown []PrescriptionLine, folded int) string {
	...
	if folded > 0 {
		...
		//
		// ★ **세 번째 정정(2026-08-12 실측) — "다음 턴에 올라온다"가 조건 없는 약속이었다.**
		// 배포 뒤 이틀치 원장에서 접힌 키 25개 중 **9개(36%)가 끝내 안 올라왔다.** 9개 전부가
		// 같은 모양이다 — **그 카드에 다음 Stop 훅이 안 왔다.** 처방은 이 훅에서만 뜨므로
		// 대화가 거기서 끝나면 접힌 것은 영영 안 나온다. 기회가 있었던 쪽은 16/16 이 바로
		// 다음 턴에 복귀했으니 기구는 멀쩡하다 — 거짓이었던 것은 **"다음 턴이 온다"** 쪽이다.
		//
		// 위 "조건을 적지 마라"와 안 부딪힌다: 이것은 조건이 아니라 **한계**다. 대화를 이어 갈
		// 사람에게는 아무 일도 안 시키고, 끝낼 사람에게만 지금 보라고 말한다. 접힘 시점에
		// 사람이 받는 신호가 이 한 줄뿐이라 여기가 아니면 적을 자리가 없다 —
		// 접힘 뒤 그 카드에 MCP 호출이 온 적은 관측 14턴 중 **0번**이라 `finish` 는 통로가 아니다.
		fmt.Fprintf(&b, "  … %d건을 접었다. 발화로 안 세므로 다음 턴에 올라온다 — 여기서 끝나면 사라진다\n", folded)
	}
	return strings.TrimRight(b.String(), "\n")
}
