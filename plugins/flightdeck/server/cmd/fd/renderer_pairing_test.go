...
//   - `add` — 위를 고치러 와서 발견했다. 그때 `RenderAdd` 는 **어느 프로젝트에 들어갔는지** ·
//     되돌리는 명령 · **"본문은 나중에 못 고친다"** 를 냈는데 `fd add` 는 한 줄만 냈다.
//     그 셋은 RenderAdd 주석이 실측 사고로 적어 둔 것이다(항목 10건 오등록, id 하나 영구 사망).
//     (셋째 문구는 그대로 안 남았다 — 2026-09-16 amend 표면이 열리며 그 문장 자체가
//     거짓이 됐고, 지금 `RenderAdd` 는 amend·note 두 수단을 안내한다. 이 문단이 남기는
//     교훈은 그 문구가 아니라 "렌더러를 통해 나가야 사람과 세션이 같은 답을 받는다"다.)
//
