...
// **관측하는** 저장소다(설계 §5 — 파생은 손 기재를 대신 읽는 것이지 쓰는 것이 아니다).
// ★ 이 줄은 오래 "(설계 §4 — 파생은 관측이지 조작이 아니다)" 였다. **§4 에 그 문장이 없다** —
// 거기 있는 "앞의 둘은 관측이지 판정이 아니다" 는 세션 상태(done)의 얘기이고 파생의 정의가
// 아니다. 파생을 정의하는 절은 §5 다. 이 레포는 이 잡이
