...
//	둘 다 **여전히 열려 있다.** 그 뒤로 바뀐 것은 각각에 대해 우리가 아는 것이다.
...
//	   ★ 이 자리를 **잠금으로 닫으려 하지 마라 — 재현이 그 처방을 반증했다**(300라운드
//	   ×6판: 오늘 286~299/300, 잠금 안으로 넣어도 284~296/300 으로 분포가 겹친다).
//	   Replay 의 첫 List·send·JudgeReplayFailure 가 전부 잠금 밖이라 "쓰겠다"는 결정이
//	   두 번 내려지고, 그 뒤에 무엇을 잠가도 append 는 두 번 난다 — 직렬화는 순서를
//	   정하지 개수를 안 바꾼다. 세는 쪽을 고쳤다(TallyRejected — doctor 가 줄 수와 고유
//	   판단 수를 갈라 낸다). 개수를 실제로 닫으려면 rejected 를 키당 파일 + O_CREATE|O_EXCL
//	   로 바꿔 중복 판정을 커널의 원자 연산으로 만들어야 한다(후속).
//	 · 잠금을 예산 안에 못 잡으면 무잠금 병합으로 떨어진다(fail-open). 큐가 깊고 세션이
//	   몰리면 실제로 일어난다 — 수치는 outbox_lock.go 의 queueLockBudget 주석에 있다.
//	   ★ **이제 그 횟수를 물을 자리가 있다** — failopen.jsonl 에 남고 fd doctor 가 낸다.
//	   전에는 o.warn 이 stderr 로만 갔고 doctor 는 파일만 읽어서, 이 한계가 얼마나
//	   밟히는지를 아무도 못 봤다. 다만 NFS 처럼 flock 이 조용히 안 걸리는 자리는 "잡았다"로
//	   나오므로 그 계수기에 **원리적으로 안 잡힌다**(doctor 가 같은 자리에서 그 사각을 찍는다).
//
