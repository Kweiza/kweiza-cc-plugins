---
name: fd-handoff
description: 작업을 마무리해 넘긴다. 판단 저장 + 후속 등록 + 항목 종료 + 자원 반납을 한 호출로. "핸드오프" · "마무리" · "세션 넘긴다" · 작업이 끝났을 때 쓴다.
---

# 마무리

도구는 `finish` 하나다. **넷이 한 트랜잭션**이라 순서를 지킬 것이 없다 —
판단만 남고 항목이 안 닫히거나, 항목은 닫혔는데 후속이 안 들어간 상태가 만들어지지 않는다.

## 한 번에 끝낸다

```
finish(
  item_id: "<집었던 항목>",
  outcome: "done",          // 또는 "dropped" — 그때는 close_reason 이 필수다
  title:   "<한 줄>",
  body:    "<아래 넷>",
  followups: [ { id, title, body, paths } ]   // 이번에 나온 후속
)
```

`body` 없이 부르면 무엇을 적어야 하는지를 **그 자리에서** 낸다.

## body 에 적을 넷

1. **왜 그렇게 했나**
2. **무엇을 기각했나**
3. **일부러 안 한 것** — 범위 밖으로 뺀 것 · 참이라 안 고친 것
4. **확인했으나 못 한 것** — 결과가 "문제 없음"이었던 조사도 여기다

git log 와 diff 가 이미 아는 것(무엇을 고쳤나)은 적지 마라.
여기 적을 것은 **그 어디에도 안 남는 것** — 판단의 이유다.

## 후속은 같은 호출에 넣는다

`followups` 로 넣으면 판단과 후속이 FK 로 이어진다. 나중에 따로 넣으면 그 연결이 없다.
다음 세션의 `pick` 이 그 판단을 항목과 함께 낸다.

## 그리고 세션을 닫는다

이 세션에서 더 할 일이 없으면 마지막에 닫는다. 안 닫으면 그 카드는 창(기본 2시간)이
지날 때까지 "지금 살아 있는 세션"에 남아, 다음 사람의 겹침 판정을 오염시킨다.

```
fd finish <항목> --body … --close     # 항목을 끝내고 세션도 닫는다
fd close [--why …]                    # 항목 없이 세션만 닫을 때
```

**선점이 남아 있으면 거절한다.** 닫힌 카드는 살아 있는 세션에서 빠지므로 그 선점이
아무에게도 안 보이게 되기 때문이다 — 항목을 아무도 못 집는데 이유도 안 보인다.
먼저 `fd finish` 로 항목을 끝내라.

닫아도 **되돌릴 수 있다** — 다음 프롬프트·도구·MCP 호출이 오면 카드가 다시 살아난다.
닫기는 판정이 아니라 관측이다.

**닫아도 안 사라지는 것이 있다.** 창을 닫고 나가거나 `tmux kill` 로 죽은 세션은 이 경로를
안 지난다(플랫폼이 프로세스 종료를 안 알려 준다). 그 카드는 창으로만 사라진다.

## 중간에 남기는 것

- `note(kind: "ask")` — 남이 건드리면 곤란한 것. **커밋 전 의도를 나르는 유일한 축**
- `note(kind: "blocked")` — 막힘. 사유가 본문이다
- `note(kind: "decision")` — 되돌리기 비싼 결정과 근거

`note` 는 응답에 **받을 세션 수**를 낸다. 0 이면 지금 아무도 안 보고 있다는 뜻이다.

## 안 하는 것

- 대시보드 손 편집 — 화면은 DB 의 뷰다
- 랜딩 sha·검증 통과 줄 적기 — 잡 레코드에서 파생된다
