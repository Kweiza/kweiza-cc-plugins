...
fd claim release --item <id> --reason "왜 끊었나" # 무신호 세션의 선점을 사람이 회수한다(항목은 open 으로)
...
재빌드를 억제하고, 재빌드는 어차피 1초 안이다. 대신 **`fd doctor` 가 말은 한다** — 옛 자리를
stat 해서 남은 개수와 크기를 찍는다:
...
`${CLAUDE_PLUGIN_DATA}` 를 그대로 명령에 쓰지 마라 — 안 걸려 있으면
