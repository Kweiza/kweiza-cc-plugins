...
**리스너 배선만 바꾼다.** `noteBuild` 는 아직 원래 자리(`serve.go:216`)에 둔다 — 의도된
서비스 동작 변화 둘은 아래에 적는다.
...
Expected: `gofmt -l` 과 vet 셋은 무출력, `go test` 는 전부 `ok`. **의도된 변화 둘은 어느 기존 시험도 안 단정하므로 기존 시험이 하나도 안 빨개져야 한다** — 빨개지면 배선이 어긋난 것이다.
