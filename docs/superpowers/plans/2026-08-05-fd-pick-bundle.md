...
- **★ 이 규율은 각 태스크가 명시한 RED 단계 하나로 끝나지 않는다. 새로 쓰거나 손댄 시험 *전부*에 대해,
  그 시험이 지키는 술어를 하나씩 망가뜨려 빨간불을 확인한다.** 이 문장이 여기 있는 이유는 실측이다 —
  Task 3 에서 이 계획이 지정한 시험 셋이 전부 무는 것이 없었다(정렬 키 ④ 를 `return false` 로,
  키 ③ 블록을 통째로 삭제, `Reason` 을 통짜 문구로 바꿔도 전 스위트가 초록이었다).
  **계획이 준 시험 코드를 그대로 옮겼다는 것은 그 시험이 문다는 근거가 아니다.**
