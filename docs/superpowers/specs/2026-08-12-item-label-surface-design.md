...
그 문장을 DESIGN §8(프로젝트 등록 절의 티클러 문단)에 적는 것이 이 설계의 답이다(§7 참조).
...
| DESIGN §8 프로젝트 등록 절의 티클러 문단 | "굶김 축 셋에서 빠진다"는 유지. **"선두에만 걸린다"를 새로 적는다** — 지금 아무 데도 안 적힌 그 사실이 사고의 절반이었다 | 조용히 낡음 |
...
| `mcpsrv/backend.go` 머리 주석 | "여기 있는 것은 **도구 7개**와 세션 귀속이 실제로 부르는 메서드뿐이다" → 8개. 인터페이스가 느는 것 자체가 그 문장의 "넓히지 않는다"와 만난다 | 조용히 낡음 |
| `cmd/fd/main.go` 의 `usage` | 새 명령 한 줄. 도움말에 없는 명령은 사람이 못 찾는다 | 조용히 낡음 |
| `api/publish_envelope_test.go` 의 봉투 표 | **Task 5 착수 뒤에 발견했다** — 이 시험이 소스의 `s.publish` 호출을 전수로 세어 표와 대조한다. 새 호출 하나가 그 표를 낡게 만든다 | **빨간불** |

★ 표 안의 "DESIGN §N" 표기는 **이 스펙의 절 번호가 아니라 DESIGN.md 의 절 번호**다.
둘은 서로 다른 좌표계인데 같은 "§N" 을 쓰다 보니 이 스펙 자신이 한 번 헷갈렸다 —
바로 위 절 제목이 `## 5. 묶음 전파` 라서 티클러 문단을 처음엔 "DESIGN §5" 로 적었는데,
DESIGN.md 의 §5 는 「파생」이고 티클러는 실제로 §8(프로젝트 등록, `.flightdeck.yaml`
스키마)에 있다. 이 표를 실행할 때는 **이 문서의 절 번호를 절대 옮겨 적지 마라** —
매번 DESIGN.md 를 직접 열어 절 번호를 다시 세라.
