...
**§5 의 소절 「경로 겹침 축 — 세 겹을 다 고친다」(`DESIGN.md:469`)의 번호 목록에 항목 4 를
순삽입한다.** 헝크 1개 · 순삽입 2~3줄 · **삭제 0** · 기존 문장 치환 0.

그 아래 "**그리고 거르지 않고 알린다**" 문단은 안 건드린다 — 이 항목은 거르는 것이 아니라
같은 목록에 겹 하나를 더하는 것이라 그 문단이 그대로 참이다.
