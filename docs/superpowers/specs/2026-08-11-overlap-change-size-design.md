...
겹침 6건 (거르지 않고 알린다 · 상대 규모 큰 순):
  ...
  · … 1건 더(제일 작은 쪽이다) — 수는 위 머리줄이 전부 센 값이다. 이름까지는 board 가 낸다
```

**절단 줄은 조건부다.** 정렬이 못 읽은 것을 맨 위로 올리므로, 상한을 넘기면 잘리는 쪽
(`in.Overlaps[tailOverlapLimit:]`)에도 규모 미상이 섞여 있을 수 있다 — git 파생이 통째로
실패하면(프로젝트 경로를 모를 때 · git 이 없을 때) 겹침 전부가 한꺼번에 그렇게 된다. 그때
"제일 작은 쪽"이라 단언하면 거짓이므로, 잘리는 쪽에 규모 미상이 하나라도 있으면 대신 이렇게
낸다:

```
  · … 1건 더(규모를 못 읽은 것이 섞여 있다 — 작다는 뜻이 아니다) — 수는 위 머리줄이 전부 센 값이다. 이름까지는 board 가 낸다
```
...
그런데 "제일 작은 쪽이다"라는 말도 무조건 쓰면 그 자체가 말 안 한 주장이 될 수 있다 —
§5.1 이 못 읽은 것을 `+∞` 로 쳐서 맨 위로 올리지만, 그것은 잘리는 쪽에 못 읽은 것이 **못
섞인다**는 보장이 아니다. 상한(`tailOverlapLimit`)보다 못 읽은 겹침이 많으면 잘리는 쪽에도
섞이고, 그때 "제일 작은 쪽"은 거짓이다. 그 판정(`judge.OverlapHasUnknownSize`)은 §5.1 의
정렬이 쓰는 것과 **같은 한 자리**를 써야 한다 — 두 자리에 각자 판정을 두면 정렬은 위로
올리는데 문구는 아래라고 말하는 어긋남이 생기고, 그 어긋남은 화면에 안 뜬다.
