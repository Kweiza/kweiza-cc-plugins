...
3. **서버/클라이언트 스큐 — 이 근거는 내가 처음에 못 봤다.**
   `fd-pick-queue-total` 세션이 같은 날 독립적으로 `PickResult` 에 포인터 필드를 더하면서
   적어 둔 것을 가져왔다. 서버는 독립 컨테이너인데 **플러그인은 자동 갱신된다** — 그래서
   구서버 + 신클라이언트 조합이 **운영자가 아무것도 안 해도** 생긴다(설계 §7 이 스큐를
   "실제로 분리 가능한 실패" 표에 넣은 이유). 그때는 **낡지도 않은 신선한 온라인 응답**에
   이 키가 없다. 값 타입이면 거기서 거짓을 단정하고, `SkewBanner` 는 `api_version`
   문자열만 보므로 이 축을 안 잡는다.

앞의 둘은 "낡은 데이터"를 겨냥하는데 셋째는 **신선한 데이터**를 겨냥한다. 축이 다르다.

> **일반화**: 두 세션이 독립적으로 같은 결론에 닿았고 근거가 부분적으로 달랐다.
> `PickResult` 에 새 필드를 더할 때는 **포인터가 기본값**으로 읽는 편이 맞다 —
> 이 구조체는 REST 를 왕복하고, 디스크 캐시로 재생되고, 버전이 갈린 두 프로세스 사이를
> 지난다. 그 세 경로 전부에서 "키 부재"와 "제로값"의 구분이 필요하다.
...
경로 실재: 이 응답은 그 축을 읽지 않았다 — 낡은 캐시이거나 서버가 이 축을 모르는 판이다.
```

마지막 줄은 `PathCheck == nil` 일 때다(§3 의 포인터 근거를 보라). 근거가 셋이므로
(낡은 디스크 캐시 · 그 필드가 생기기 전의 캐시 · 신클라이언트+구서버 스큐) 문구도
셋을 다 아우른다 — "낡은 캐시일 수 있다"로만 좁히면 셋째(신선한 응답인데 서버가
이 축을 모르는 경우)를 오독시킨다.
