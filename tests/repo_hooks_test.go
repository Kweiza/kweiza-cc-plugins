...
// 이 저장소의 훅 관문 — 플러그인들에 Go 코드가 없어서 이 모듈에 따로 산다.
//
// ★ **이 파일이 여기 있는 이유.** 이 관문은 원래 flightdeck 의 Go 모듈
// (`plugins/flightdeck/server/cmd/fd/repo_hooks_test.go`) 안에 얹혀 **저장소 전체**를 물었다 —
// 옆 플러그인들에는 Go 코드가 없어 다른 시험이 없었기 때문이다. 2026-09-24 에 flightdeck 이
// 단독 저장소(Kweiza/flightdeck)로 분리되면서 그 모듈도 떠났고, 그대로 두면 이 저장소의
// 관문이 0 이 된다. 그래서 flightdeck 몫을 뺀 나머지를 독립 모듈(`tests/`)로 옮겨 왔다.
// flightdeck 자신의 훅은 이제 그 저장소의 시험이 문다.
//
// ★ **훅이 죽는 모양은 오류가 아니라 부재다.** 훅 경로가 틀리면 세션이 그냥 아무것도 안 한다.
// grafik-bar 는 **SessionStart 훅으로 스스로 설치되는** 플러그인이라(`plugin.json` 의
// description 이 그렇게 선언한다), 그 경로가 틀리면 상태줄이 안 뜨는 것으로만 나타난다.
// 화면에 아무 오류도 안 뜬다.
//
// ★ **잠글 것의 근거는 어디서 오나.** 이 저장소의 플러그인에는 설계 문서가 없다 — 그래서
// 이 파일이 무는 것은 근거가 **둘 중 하나**에서 오는 축뿐이다:
//
...
//
// ★ 플랫폼 실측 표(`platformHookEvents`·`platformMatcherValues`)는 flightdeck 저장소에도
// 같은 것이 있다 — 분리 시점에 두 벌이 됐다. 플랫폼이 움직여 한쪽을 고치면 다른 쪽도 재라.
...
// flightdeck DESIGN §6 이 "훅 이벤트 31종에도 프로세스 종료를 알리는 것이 없다"고 적은 그 31 과 같은 수다.
//
...
//
// ★ `SessionStart` 에 **`fork` 가 있다**. flightdeck DESIGN §6 의 표(2.1.221·2.1.222 실측)에는
// 이 값이 없었다 — 플랫폼이 움직인 자리다. 세션 전환 사유 여덟
...
//
// ★ `SessionEnd`·`PreCompact` 는 지금 이 저장소의 어느 훅도 안 쓴다. 열거값은 플랫폼 실측이라
// 남겨 둬서, 그 훅이 생기는 날 **값의 오타**는 바로 잡힌다. 완전성(complete)은 안 문다 —
// 요구할 근거는 그 훅이 무엇을 하느냐에서 오므로, 훅을 만들 때 그 근거와 함께 정하라.
var platformMatcherValues = map[string]matcherSpec{
	"SessionStart": {
		values:   []string{"clear", "compact", "fork", "resume", "startup"},
		complete: true,
		why: "이 저장소의 SessionStart 훅(grafik-bar 의 setup-statusline.sh)은 **갈래를 안 가린다** — " +
			"멱등이라 어느 갈래로 시작하든 같은 일을 한다. 그래서 빠진 값은 의도가 아니라 표류이고, " +
			"그 갈래로 시작한 세션은 상태줄이 없다",
	},
	"SessionEnd": {
		values:   []string{"clear", "logout", "other", "prompt_input_exit", "resume"},
		complete: false,
		why:      "이 저장소의 훅은 SessionEnd 를 안 쓴다 — 값의 오타만 문다",
	},
	"PreCompact": {
		values:   []string{"auto", "manual"},
		complete: false,
		why:      "이 저장소의 훅은 PreCompact 를 안 쓴다 — 값의 오타만 문다",
	},
}
...
// 그날 할 일은 이 단정을 지우는 것이 아니라 `hooksFile`(root_test.go)에 그 필드를 더하고
		...
		for ev, groups := range hf.Hooks {
			for gi, g := range groups {
				...
				for hi, hk := range g.Hooks {
					...
					// ★ 절대경로여야 한다. 훅 실행 환경이 Bash 도구와 같다는 보장이 없다(flightdeck DESIGN §13).
...
// 도구가 없는 머신에서 관문이 아무것도 안 물면서 초록인 것은 통과가 아니다.
...
// isShellScript 는 첫 줄의 shebang 으로 셸 스크립트인지 본다(확장자가 없는 스크립트용).
...
//
// ★★ **값이 아니라 존재를 문다.** 이 저장소의 플러그인에는 설계 문서가 없다 — 그래서
// "정확히 N 초"를 여기서 잠그지 않는다. 근거가 안 따라오는 수를 관문에 들이면 그것은 관문이
// 아니라 장식이다(스킬 줄 수 상한을 이 모듈로 안 가져온 것과 같은 규율, `repo_skills_test.go`
// 머리말).
//
...
// SessionStart 훅은 **사람이 첫 글자를 치기 전에** 끝나야 하고, grafik-bar 의 훅은 그 자리에서
// 사용자가 켠 다른 플러그인(flightdeck 등)의 SessionStart 훅과 **함께** 돈다 — 합이 곧 세션이
// 안 뜨는 시간이다.
