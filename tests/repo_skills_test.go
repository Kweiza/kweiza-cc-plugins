...
// 이 저장소의 스킬 관문 — 플러그인들에 Go 코드가 없어서 이 모듈에 따로 산다.
//
// ★ **이 파일이 여기 있는 이유.** 이 관문은 원래 flightdeck 의 Go 모듈
// (`plugins/flightdeck/server/cmd/fd/repo_skills_test.go`) 안에 얹혀 저장소 전체의 스킬을
// 물었다. 2026-09-24 에 flightdeck 이 단독 저장소(Kweiza/flightdeck)로 분리되면서 그 모듈도
// 떠났다. 그래서 flightdeck 몫을 뺀 나머지를 독립 모듈(`tests/`)로 옮겨 왔다.
//
...
// 그때 손으로 잰 것을 굳힌 것이다.
//
// ★ **줄 수 상한은 여기 없다.** flightdeck 의 스킬 줄 수 상한은 "부른 턴의 예산 × 호출
// 빈도"가 근거였고 그 빈도는 flightdeck 스킬의 것이다. `session-handoff` 는 세션당 많아야
// 한 번 부르고 그 산문이 인수인계 문서의 형식을 나르므로 같은 회귀선이 안 선다. 빼먹은 것이
// 아니라 **근거가 안 따라오는 것**이다 — 상한을 걸려면 그 근거부터 새로 세워라.
...
//
// ★ **이 수는 물려받은 미측정값이다** — flightdeck DESIGN §13 「아직 아님」 6 이 "항목당
// 1,536자 절단"에 측정 기록이 없다고 적어 뒀다. 그래서 이것은 플랫폼 한계의 재현이 아니라
// **폭주 방지선**이다: 평상시엔 아무것도 안 문다. 여기에 걸리는 날 할 일은 상한을 올리는
// 것이 아니라 **그 줄을 재는 것**이다.
//
// 자(rune)로 센다 — 그 문서가 "1,536자"라 적었고, 그 수가 바이트라는 근거도 없기
	...
	for _, s := range skillRefs(t, root) {
		...
		if n := utf8.RuneCountInString(desc); n > skillDescRuneCap {
			t.Fatalf("%s 의 description 이 %d자(%d바이트)다 — 상한 %d자다.\n"+
				"이 상한은 flightdeck DESIGN §13 「아직 아님」 6 의 **미측정값**이다. 여기 걸렸으면 상한을\n"+
...
//
// ★★ **분리 뒤의 사각.** 위 예의 한쪽(flightdeck 의 `fd-handoff`)은 이제 다른 저장소에 있어
// 이 관문이 못 본다. 두 플러그인을 함께 켠 머신에서는 여전히 같은 목록에 뜬다 — 이쪽 트리거를
// 고칠 때는 Kweiza/flightdeck 의 스킬 description 도 함께 봐라.
